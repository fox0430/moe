...
suite "osext: isPath":
  ...
  proc isPathTest(testIndex: int, expectVal: bool, line: Runes) =
...
suite "osext: splitPathExt":
  ...
  proc splitPathExtTest(
      testIndex: int, expectVal: tuple[head, tail: Runes], path: Runes
  ) =
  ...
  const TestCases: seq[tuple[expectVal: tuple[head, tail: Runes], path: Runes]] = @[
