...
suite "diffviewer: newDiffViewerState":
...
suite "diffviewer: classifyDiffLine":
...
suite "diffviewer: getSelectedLine":
...
suite "diffviewer: initDiffViewerBuffer - Integration tests":
  setup:
    let testDir = getTempDir() / "moe_diffviewer_test"
...
suite "diffviewer: initDiffViewerState - Integration tests":
  setup:
    let testDir = getTempDir() / "moe_diffviewer_state_test"
    if dirExists(testDir):
      removeDir(testDir)
    createDir(testDir)
...
suite "diffviewer: Movement edge cases":
...
suite "diffviewer: DiffLineKind enum":
...
suite "diffviewer: classifyDiffLine - Additional cases":
...
suite "diffviewer: moveDown - topLine behavior":
...
suite "diffviewer: initDiffViewerBuffer - Additional integration tests":
  setup:
    let testDir = getTempDir() / "moe_diffviewer_additional_test"
    if dirExists(testDir):
      removeDir(testDir)
    createDir(testDir)
...
suite "diffviewer: initDiffViewerState - Error message validation":
  setup:
    let testDir = getTempDir() / "moe_diffviewer_error_test"
    if dirExists(testDir):
      removeDir(testDir)
    createDir(testDir)
...
suite "diffviewer: DiffViewerState field access":
