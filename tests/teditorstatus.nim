...
test "resize 1":
  ...
  status.highlight = initHighlight($status.buffer, status.language, status.settings.editorColor.editor)
  status.view.add(initEditorView(status.buffer, 1, 1))
  status.resize(0, 0)
...
test "resize 2":
  ...
  status.buffer = initGapBuffer(@[ru"a"])
  status.highlight = initHighlight($status.buffer, status.language, status.settings.editorColor.editor)
  status.view.add(initEditorView(status.buffer, 20, 4))
  status.resize(20, 4)
