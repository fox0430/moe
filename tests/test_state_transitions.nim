...
import
  moepkg/[
    editorstatus, bufferstatus, unicodeext, windownode, gapbuffer, viewhighlight,
    registers, visualmode, commandline,
  ]
...
suite "State Transition Consistency Tests":
  ...
  test "Search state transition consistency":
    ...
    check status.highlightingText.isSome or status.highlightingText.isNone
      # Either is valid
  ...
  test "Command line state transition consistency":
    ...
    check status.commandLine.buffer.len == 0 or status.commandLine.buffer.len > 0
      # Buffer exists
