...
import
  ui, gapbuffer, unicodeext, windownode, bufferstatus, movement, messages, settings,
  registers, commandline, independentutils, searchutils, completion
...
proc isOpenParen(ch: char): bool {.inline.} =
  ch in {'(', '{', '[', '\"', '\''}

proc isCloseParen(ch: char): bool {.inline.} =
  ch in {')', '}', ']', '\"', '\''}
...
proc isEmptyLine(bufStatus: BufferStatus, windowNode: WindowNode): bool {.inline.} =
...
proc getRegister(r: var Registers, name: string = ""): Register =
  ...
  if name.len == 0:
    return r.getNoNamedRegister
  elif name.isNamedRegisterName:
    return r.getNamedRegister(name)
  elif name.isNumberRegisterName:
    return r.getNumberRegister(name)
  elif name.isSmallDeleteRegisterName:
    return r.getSmallDeleteRegister
  elif name.isClipBoardRegisterName:
    return r.getClipBoardRegister

proc insertCharacter*(
    bufStatus: var BufferStatus, windowNode: WindowNode, autoCloseParen: bool, c: Rune
) =
...
proc bufferPositionsForMultipleEdit*(
    selectedArea: SelectedArea, column: int
): seq[BufferPosition] =
...
proc bufferPositionsForMultipleEdit*(
    bufStatus: BufferStatus, column: int
): seq[BufferPosition] {.inline.} =
  ## Return positions for multiple positions edtting.

  bufferPositionsForMultipleEdit(bufStatus.selectedArea.get, column)

proc insertMultiplePositions*(
    bufStatus: var BufferStatus, positions: seq[BufferPosition], r: Runes | Rune
) =
  ...
  for i, p in positions:
    if beforeLine != p.line:
      addedCol = 0

    let colNum = p.column + addedCol

    if p.line <= bufStatus.buffer.len and colNum <= bufStatus.buffer[p.line].len:
  ...
  if isChanged:
    ...
    if not bufStatus.isUpdate:
      bufStatus.isUpdate = true

proc deleteParen*(
    bufStatus: var BufferStatus, currentLine, currentColumn: int, currentChar: Rune
) =
  ...
  if isOpenParen(currentChar):
    ...
    for i in currentLine ..< buffer.len:
      for j in currentColumn ..< buffer[i].len:
        if buffer[i][j] == openParen:
          inc(depth)
        elif buffer[i][j] == closeParen:
          dec(depth)
        if depth == 0:
    ...
    for i in countdown(currentLine, 0):
      ...
      for j in countdown(startColumn, 0):
        if buffer[i][j] == closeParen:
          inc(depth)
        elif buffer[i][j] == openParen:
          dec(depth)
        if depth == 0:
          var line = bufStatus.buffer[i]
          line.delete(j)
...
proc currentLineDeleteCharacterBeforeCursor(
    bufStatus: var BufferStatus, windowNode: WindowNode, autoDeleteParen: bool
) =
  if windowNode.currentLine == 0 and windowNode.currentColumn == 0:
    return
  ...
  if autoDeleteParen:
    bufStatus.deleteParen(windowNode.currentLine, windowNode.currentColumn, currentChar)

  if (
    bufStatus.mode == Mode.insert and
...
proc deleteMultiplePositions*(
    bufStatus: var BufferStatus, positions: seq[BufferPosition], numOfDelete: int
) =
...
proc keyBackspace*(
  ...
  if windowNode.currentColumn == 0:
    ...
    let
      ...
      numOfDelete =
        if numOfSpsce == 0 or currentColumn > numOfSpsce:
          1
...
proc deleteBeforeCursorToFirstNonBlank*(
    bufStatus: var BufferStatus, windowNode: WindowNode
) =
  if windowNode.currentColumn == 0:
    return

  let firstNonBlank = bufStatus.getFirstNonBlankOfLineOrFirstColumn(windowNode)
...
proc insertIndentWhenPairOfParen(
  ...
  if bufStatus.language != SourceLanguage.langNim and
    ...
    if isCloseParen(nextLine[^1]) and isCorrespondingParen(openParen, nextLine[^1]):
...
proc insertIndentInNimForKeyEnter(
  ...
  if line.len > 0:
    ...
    if currentColumn == line.len and (
      (
        line.len > 2 and line.splitWhitespace == @[ru"var"] or
        line.splitWhitespace == @[ru"let"]
      ) or (line.len > 4 and line.splitWhitespace == @[ru"const"]) or (
        line.len > 4 and line.splitWhitespace.len > 0 and
        line.splitWhitespace[^1] == (ru"object")
...
proc insertIndentForKeyEnter(
    bufStatus: var BufferStatus, winNode: WindowNode, autoIndent: bool, tabStop: int
) =
  ...
  of SourceLanguage.langYaml:
    ...
    bufStatus.insertIndentInPlainTextForKeyEnter(winNode, autoIndent, tabStop)
...
proc insertCharacterBelowCursor*(bufStatus: var BufferStatus, windowNode: WindowNode) =
  let
    currentLine = windowNode.currentLine
    currentColumn = windowNode.currentColumn
    buffer = bufStatus.buffer

  if currentLine == bufStatus.buffer.high:
    return
  if currentColumn > buffer[currentLine + 1].high:
    return
...
proc insertCharacterAboveCursor*(bufStatus: var BufferStatus, windowNode: WindowNode) =
  let
    currentLine = windowNode.currentLine
    currentColumn = windowNode.currentColumn
    buffer = bufStatus.buffer
...
proc deleteWord*(
  ...
  for i in 0 ..< loop:
    if bufStatus.buffer.len == 1 and bufStatus.buffer[windowNode.currentLine].len < 1:
      return
    elif bufStatus.buffer.len > 1 and windowNode.currentLine < bufStatus.buffer.high and
      ...
      let
        ...
        currentColumn = windowNode.currentColumn
        startWith =
          if bufStatus.buffer[currentLine].len == 0:
            ru '\n'
          else:
            bufStatus.buffer[currentLine][currentColumn]
        isSkipped =
          if unicodeext.isPunct(startWith):
            unicodeext.isPunct
          elif unicodeext.isAlpha(startWith):
            unicodeext.isAlpha
          elif unicodeext.isDigit(startWith):
            unicodeext.isDigit
          else:
            nil
      ...
      while true:
        ...
        if isPunct(curr) or isAlpha(curr) or isDigit(curr) or
            (not withSpace and isSpace(curr)):
          break
        inc(windowNode.currentColumn)
...
proc deleteWordBeforeCursor*(
  ...
  if windowNode.currentColumn == 0:
    ...
    bufStatus.keyBackspace(windowNode, isAutoDeleteParen, settings.standard.tabStop)
  else:
    bufStatus.moveToBackwardWord(windowNode)

    const WithSpace = true
    bufStatus.deleteWord(windowNode, loop, WithSpace, registers, registerName, settings)
...
proc deleteWordBeforeCursor*(
  ...
  const RegisterName = ""
  bufStatus.deleteWordBeforeCursor(windowNode, registers, RegisterName, loop, settings)
...
proc indent*(bufStatus: var BufferStatus, windowNode: WindowNode, tabStop: int) =
...
proc unindent*(bufStatus: var BufferStatus, windowNode: WindowNode, tabStop: int) =
  ...
  if numOfDeleteSpace > 0:
    ...
    if oldLine != newLine:
      ...
      if newLine.high < windowNode.currentColumn:
        windowNode.currentColumn = if newLine.high == -1: 0 else: newLine.high

proc deleteCharactersBeforeCursorInCurrentLine*(
    bufStatus: var BufferStatus, windowNode: var WindowNode
) =
  if windowNode.currentColumn == 0:
    return
...
proc indentInCurrentLine*(
    bufStatus: var BufferStatus, windowNode: WindowNode, tabStop: int
) =
...
proc deleteCharacter*(
    bufStatus: var BufferStatus, line, colmun: int, autoDeleteParen: bool
) =
  let
    currentLine = line
  ...
  if currentLine >= bufStatus.buffer.high and
      currentColumn > bufStatus.buffer[currentLine].high:
    return
  ...
  if currentColumn == bufStatus.buffer[currentLine].len:
    ...
    if autoDeleteParen and currentChar.isParen:
      bufStatus.deleteParen(line, colmun, currentChar)
...
proc deleteCharacters*(
  ...
  if line >= bufStatus.buffer.high and colmun > bufStatus.buffer[line].high:
    return
  ...
  for i in 0 ..< loop:
    ...
    if currentColumn > newLine.high:
      currentColumn = newLine.high

    if settings.standard.autoDeleteParen and deleteChar.isParen:
      bufStatus.deleteParen(line, colmun, deleteChar)
...
proc deleteCharacters*(
    bufStatus: var BufferStatus, autoDeleteParen: bool, line, colmun, loop: int
) =
  ## No yank buffer
...
proc insertIndentNimForOpenBlankLine(
    bufStatus: var BufferStatus, windowNode: var WindowNode, tabStop: int
) =
  ...
  if aboveLine.len > 0:
    ...
    # And, if finish the current line with ':', "object, '='"
    if (
      aboveLine.splitWhitespace == @[ru"var"] or aboveLine.splitWhitespace == @[ru"let"] or
      aboveLine.splitWhitespace == @[ru"const"] or
      aboveLine.splitWhitespace[^1] == (ru"object") or aboveLine[^1] == (ru ':') or
      aboveLine[^1] == (ru '=')
    ):
      let
      ...
      (aboveLine.len > 2 and aboveLine.splitWhitespace[^1] == ru "or") or
...
proc deleteLines*(
  ...
  if bufStatus.buffer.len == 0:
    bufStatus.buffer.insert(ru"", 0)

  if startLine < windowNode.currentLine:
    dec(windowNode.currentLine)
...
proc deleteCharacterUntilEndOfLine*(
  ...
  bufStatus.deleteCharacters(
    registers, registerName, currentLine, startColumn, loop, settings
  )
...
proc deleteCharactersAfterBlankInLine*(
  ...
  bufStatus.deleteCharacters(
    registers, registerName, currentLine, firstNonBlankCol, loop, settings
  )
  ...
  block:
    ...
    if currentColumn > 0:
      ...
      for i in 0 ..< currentColumn:
        deletedLine.add oldLine[i]
      if deletedLine.len > 0:
...
proc deleteTillNextBlankLine*(
  ...
  if blankLine < 0:
    blankLine = bufStatus.buffer.len
  ...
  block:
    ...
    if currentColumn > 0:
      var deletedLine: Runes
      for i in currentColumn ..< oldLine.len:
        deletedLine.add oldLine[i]
...
proc yankLines*(
  ...
  commandLine.writeMessageYankedLine(yankedBuffer.len, notificationSettings)
...
proc yankLines*(
  ...
  const Name = ""
  bufStatus.yankLines(
    registers, commandLine, notificationSettings, first, last, Name, isDelete, settings
  )
...
proc yankCharacters*(
  ...
  commandLine.writeMessageYankedCharacter(yankedBuffer.len, settings.notification)
  ...
  for i in 0 ..< loop:
    ...
    for j in startColumn ..< line.len:
      ...
      if isWhiteSpace(rune):
        for k in j ..< line.len:
          if isWhiteSpace(line[k]):
            yankedBuffer.add rune
          else:
            startColumn = k
            break
        break
...
proc yankWord*(
  ...
  const Name = ""
  bufStatus.yankWord(registers, windowNode, loop, Name, isDelete, settings)
...
proc insertRunesFromRegister(
    bufStatus: var BufferStatus, position: BufferPosition, register: Register
) =
...
proc insertLinesFromRegister(
    bufStatus: var BufferStatus, position: int, register: Register
) =
  ## Get buffer from the register.
  ...
  for i in 0 ..< register.buffer.len:
    bufStatus.buffer.insert(register.buffer[i], position + i)
...
proc pasteAfterCursor*(
    bufStatus: var BufferStatus, windowNode: var WindowNode, register: Register
) =
  ...
  if register.buffer.len > 0:
    if register.isLine:
      ...
      let
        ...
        insertColumn =
          if bufStatus.isEmptyLine(windowNode):
            0
          else:
            windowNode.currentColumn + 1
        position = BufferPosition(line: windowNode.currentLine, column: insertColumn)
...
proc replaceOnlyFirstWordInLines*(
    b: var BufferStatus, lineRange: Range, sub, by: Runes
) =
...
proc toggleCharacters*(
    bufStatus: var BufferStatus, windowNode: var WindowNode, loop: int
) =
  let oldLine = bufStatus.buffer[windowNode.currentLine]
  var newLine = bufStatus.buffer[windowNode.currentLine]
...
proc autoIndentCurrentLine*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  let currentLine = windowNode.currentLine

  if currentLine == 0 or bufStatus.buffer[currentLine].len == 0:
    return
...
proc joinLine*(bufStatus: var BufferStatus, windowNode: WindowNode) =
  if windowNode.currentLine == bufStatus.buffer.len - 1 or
      bufStatus.buffer[windowNode.currentLine + 1].len < 1:
    return
...
proc undo*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  if not bufStatus.buffer.canUndo:
    return

  bufStatus.buffer.undo
  windowNode.revertPosition(bufStatus.positionRecord, bufStatus.buffer.lastSuitId)
...
proc deleteInsideOfParen*(
  ...
  let
    ...
    oldLine = bufStatus.buffer[currentLine]
    openParen =
      if isCloseParen(rune):
        correspondingOpenParen(rune)
      else:
        rune
    closeParen =
      if isOpenParen(rune):
        correspondingCloseParen(rune)
      else:
        rune
...
proc deleteInsideOfParen*(
  ...
  const RegisterName = ""
  bufStatus.deleteInsideOfParen(windowNode, registers, RegisterName, rune, settings)

proc getWordUnderCursor*(
    bufStatus: BufferStatus, windowNode: WindowNode
): (int, Runes) =
...
proc getCharacterUnderCursor*(bufStatus: BufferStatus, windowNode: WindowNode): Rune =
  let line = bufStatus.buffer[windowNode.currentLine]
  if line.len() <= windowNode.currentColumn:
    return
...
proc modifyNumberTextUnderCurosr*(
    bufStatus: var BufferStatus, windowNode: var WindowNode, amount: int
) =
  ...
  if not isDigit(bufStatus.buffer[currentLine][currentColumn]):
    return
