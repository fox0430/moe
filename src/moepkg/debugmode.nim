...
import ui, unicodeext, windownode, bufferstatus, movement, commandline, editorstatus
...
proc isDebugModeCommand*(command: Runes): InputState =
  ...
  if command.len == 1:
    let key = command[0]
    if key == ord(':') or isCtrlK(key) or isCtrlJ(key) or key == ord('k') or isUpKey(
      key
    ) or key == ord('j') or isDownKey(key) or key == ord('g') or key == ord('G'):
