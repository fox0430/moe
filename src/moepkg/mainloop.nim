...
import
  editorstatus, bufferstatus, windownode, unicodeext, gapbuffer, ui, normalmode,
  visualmode, insertmode, exmode, filermode, replacemode, buffermanager, recentfilemode,
  quickrun, backupmanager, configmode, debugmode, commandline, search, commandlineutils,
...
proc invokeCommand(
    currentMode: Mode, command: Runes, recodingOperationRegister: Option[Rune]
): InputState =
  case currentMode
...
proc assignNextSearchHistory(
    status: var EditorStatus, searchHistoryIndex: var Option[int]
) =
...
proc isExecMacroCommand(
  ...
  if bufStatus.mode.isNormalMode and commands.len > 1:
    if commands[0].isDigit:
      ...
      # repetitions.
      var i = 0
      while i < commands.high and commands[i].isDigit:
        i.inc
      if commands.len - i == 2:
        return
          commands[i] == ord('@') and isOperationRegisterName(commands[i + 1]) and
          registers.getOperations(commands[i + 1]).get.commands.len > 0
    else:
      return
        commands.len == 2 and commands[0] == ord('@') and
...
proc execEditorCommand(status: var EditorStatus, command: Runes): Option[Rune] =
  ...
  if status.recodingOperationRegister.isSome:
    if not isStopRecordingOperationsCommand(currentBufStatus, command):
      discard
        status.registers.addOperation(status.recodingOperationRegister.get, command).get
      ...
      for lineNum, line in pasteBuffer:
        for r in line:
          currentBufStatus.replaceCurrentCharAndMoveToRight(
            currentMainWindowNode, status.settings.standard.autoCloseParen, r
          )
        if lineNum < pasteBuffer.high:
...
proc jumpAndHighlightInReplaceCommand(status: var EditorStatus) =
  ...
  if info.sub.len > 0 and info.by.len == 0:
    status.highlightingText =
      HighlightingText(
        kind: HighlightingTextKind.replace, text: info.sub.replaceToNewLines.splitLines
      ).some

    currentBufStatus.jumpToSearchForwardResults(
      currentMainWindowNode, info.sub, status.settings.standard.ignorecase,
...
proc isReplaceCommand(status: EditorStatus): bool {.inline.} =
  ...
  currentBufStatus.isExMode and status.commandLine.buffer.startsWith(ru"%s/") and
...
proc isIncrementalReplace(status: EditorStatus): bool {.inline.} =
  status.settings.standard.incrementalSearch and status.isReplaceCommand

proc initBeforeLineForIncrementalReplace(status: var EditorStatus): seq[BeforeLine] =
  let info = parseReplaceCommand(status.commandLine.buffer)
  if info.sub.len > 0 and info.by.len > 0:
    let positions =
      currentBufStatus.buffer.toSeqRunes.searchAllOccurrence(info.sub, false, false)
    for p in positions:
      if result.len == 0 or result[^1].lineNumber != p.line:
        result.add BeforeLine(
          lineNumber: p.line, lineBuffer: currentBufStatus.buffer[p.line]
        )

proc isUpdateIncReplceInfo(
    incReplaceInfo: IncrementalReplaceInfo, replaceCommandInfo: ReplaceCommandInfo
): bool {.inline.} =
...
proc restoreLinesFromBeforeLines(
    bufStatus: var BufferStatus, beforeLines: seq[BeforeLine]
) {.inline.} =
...
proc execIncrementalReplace(
    status: var EditorStatus, incReplaceInfo: IncrementalReplaceInfo
) {.inline.} =
  status.replaceBuffer(
    (sub: incReplaceInfo.sub, by: incReplaceInfo.by, isGlobal: incReplaceInfo.isGlobal)
  )

proc incrementalReplace(
    status: var EditorStatus, incReplaceInfo: var Option[IncrementalReplaceInfo]
) =
  ...
  if incReplaceInfo.isNone:
    ...
    currentBufStatus.restoreLinesFromBeforeLines(incReplaceInfo.get.beforeLines)
...
template isOpenCompletionWindowInCommandLine(status: EditorStatus, key: Rune): bool =
  status.completionwindow.isNone and currentBufStatus.isExmode and
    status.settings.standard.popupWindowInExmode and isCompletionCharacter(key)

proc openCompletionWindowInCommandLine(
    status: var EditorStatus, isCurrentPosition: bool = false
) =
...
proc confirmCompletion(status: var EditorStatus) =
  ...
  if not currentBufStatus.isExmode and currentBufStatus.isEditMode and
...
proc confirmCompletionAndContinue(status: var EditorStatus) =
  ...
  status.completionWindow.get.inputText = status.completionWindow.get.selectedText
  status.completionWindow.get.selectedIndex = -1

template isConfirmCompletionAndContinue(status: EditorStatus, key: Rune): bool =
  isCompletionCharacter(key) and status.completionWindow.get.selectedIndex > -1
...
proc updateCompletionWindowBufferInCommandLine(status: var EditorStatus) =
  ...
  let
    ...
    maxPosition = Position(y: status.commandLine.window.y, x: getTerminalWidth() - 1)
  ...
  if currentBufStatus.isExmode:
    ...
    status.completionWindow.get.setList initDocSymbolCompletionList(
      currentBufStatus.documentSymbols, status.commandLine.buffer
    )
  ...
  if status.completionWindow.get.list.len > 0:
    ...
    status.completionWindow.get.setWindowPositionY(status.commandline.windowPosition.y)
...
proc commandLineLoop*(status: var EditorStatus): Option[Rune] =
  ...
  proc isJumpAndHighlightInReplaceCommand(status: EditorStatus): bool {.inline.} =
    status.settings.standard.incrementalSearch and currentBufStatus.isExMode and
  ...
  while not isCancel:
    ...
    if status.completionWindow.isSome:
      if canHandleInCompletionWindow(key):
        status.completionWindow.get.handleKey(status.commandLine, key)
        continue
      else:
        if isEnterKey(key):
          if status.completionWindow.get.selectedIndex == -1:
    ...
    if status.settings.standard.incrementalSearch and currentBufStatus.isSearchMode:
    ...
    if not isClosedCompletionWindow and status.isOpenCompletionWindowInCommandLine(key):
  ...
  if isCancel:
    ...
    if isExMode(currentBufStatus.mode):
      ...
      let inputState =
        invokeCommand(currentBufStatus.mode, command, status.recodingOperationRegister)
      case inputState
      of InputState.Valid:
...
template isOpenCompletionWindowInEditor(status: EditorStatus, key: Rune): bool =
  status.completionwindow.isNone and currentBufStatus.isInsertMode and
    status.settings.autocomplete.enable and isCompletionCharacter(key)
...
template updateCompletionWindowHighlightingText(status: var EditorStatus) =
  ...
  elif status.highlightingText.isNone and status.completionWindow.get.isHighlightingText:
    # Clear highlight
...
proc updateCompletionWindowBufferInEditor(status: var EditorStatus) =
  ...
  if status.completionWindow.get.selectedIndex > -1:
    ...
    currentMainWindowNode.currentColumn = status.completionWindow.get.startColumn

  let
    # Calc min/max positions for the completion window.
    minPosition = Position(y: currentMainWindowNode.y, x: currentMainWindowNode.x)
...
template isUpdateCompletionWindow(
    status: EditorStatus, beforeWaitingResponse: Option[WaitLspResponse]
): bool =
  status.completionWindow.isSome and beforeWaitingResponse.isSome and
    beforeWaitingResponse.get.lspMethod == LspMethod.textDocumentCompletion and
    not lspClient.isWaitingResponse(
      currentBufStatus.id, LspMethod.textDocumentCompletion
    )
...
template isSendLspRequests(status: var EditorStatus): bool =
  currentBufStatus.isEditMode and status.lspClients.contains(currentBufStatus.langId) and
    lspClient.isInitialized and now() > status.lastOperatingTime + 500.milliseconds

proc inViewRange(status: EditorStatus): bool =
  let viewRange = currentMainWindowNode.view.rangeOfOriginalLineInView
  return
    viewRange.first >= currentBufStatus.inlayHints.range.first and
    viewRange.last <= currentBufStatus.inlayHints.range.last

proc isSendLspInlayHintRequest(status: var EditorStatus): bool {.inline.} =
  return
    lspClient.capabilities.get.inlayHint and not status.inViewRange and
    not lspClient.isWaitingResponse(
      currentBufStatus.id, LspMethod.textDocumentInlayHint
    )

proc isSendLspInlineValueRequest(status: var EditorStatus): bool {.inline.} =
  return
    lspClient.capabilities.get.inlineValue and not status.inViewRange and
    not lspClient.isWaitingResponse(
      currentBufStatus.id, LspMethod.textDocumentInlineValue
    )
...
template isSendLspDocumentHighlightRequest(status: var EditorStatus): bool =
  lspClient.capabilities.get.documenthighlight and currentBufStatus.isNormalMode and
    currentBufStatus.documentHighlightInfo.ranges.len == 0 and
    not lspClient.isWaitingResponse(
      currentBufStatus.id, LspMethod.textDocumentDocumentHighlight
    )
...
proc sendLspRequests(status: var EditorStatus) =
  ...
  if status.isSendLspDocumentHighlightRequest:
    currentBufStatus.documentHighlightInfo =
      DocumentHighlightInfo(position: currentMainWindowNode.bufferPosition)
...
template isUpdateLogViewer(status: var EditorStatus): bool =
  status.isUpdateEditorLogViewer or status.isUpdateLspLogViewer

proc isClearDocumentHighlightInfo(status: var EditorStatus): bool =
  return
    status.isLspClientInitialized and
    currentBufStatus.documentHighlightInfo.ranges.len > 0 and
    (not status.inRangeLspdocumentHighlight or not currentBufStatus.isNormalMode)
        ...
        if status.completionWindow.isSome:
          if canHandleInCompletionWindow(key.get):
            status.completionWindow.get.handleKey(
              currentBufStatus, currentMainWindowNode, key.get
            )
            key.resetKeyAndContinue
          else:
