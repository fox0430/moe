...
type
  ...
  Registers* = ref object
    ...
    noNamed: NoNamedRegister ## The latest updating register buffer.
...
proc setNoNamedRegister*(r: var Registers, buffer: Runes | seq[Runes]) {.inline.} =
...
proc set(r: var NamedRegisters, buffer: Runes | seq[Runes], registerName: char) =
  ## set the named register.
  ## Register names: 'A' ~ 'Z', 'a' ~ 'z'.

  doAssert(registerName in Letters, fmt"Named register: Invalid name: {registerName}")

  r[registerName].set(buffer)

proc setSmallDeleteRegister*(r: var Registers, buffer: Runes) {.inline.} =
...
proc setYankedRegister*(r: var Registers, buffer: Runes | seq[Runes]) =
...
proc setDeletedRegister*(r: var Registers, buffer: Runes) {.inline.} =
  ## set the small delete register, no named register and OS clipboard.

  r.setSmallDeleteRegister(buffer)
  r.setNoNamedRegister(buffer)

proc setDeletedRegister*(r: var Registers, buffer: seq[Runes]) =
...
proc setDeletedRegister*(r: var Registers, buffer: seq[Runes], registerNumber: int) =
  ## set the number register for deleted lines (Register '1'), no named
  ## register and OS clipboard.
...
proc setNamedRegister*(
    r: var Registers, buffer: Runes | seq[Runes], registerName: char
) {.inline.} =
...
proc setNamedRegister*(
    r: var Registers, buffer: Runes | seq[Runes], registerName: Rune
) =
  ## set the named register, no named register and OS clipboard.
  ## Register numbers: 'A' ~ 'Z', 'a' ~ 'z'.

  doAssert(
    r.canConvertToChar, fmt"Named register: Invalid register name: {registerName}"
  )
...
proc isUpdateClipBoardRegister(
    r: Registers, clipboardBuffer: seq[Runes]
): bool {.inline.} =
...
proc getSmallDeleteRegister*(r: Registers): Register {.inline.} =
  r.smallDelete
...
proc addNormalModeOperation*(r: var Registers, command: Runes) {.inline.} =
  ## Add an operation to normalModeOperationsRegister.

  r.normalModeOperations.commands.add command

proc getNormalModeOperations*(r: Registers): NormalModeOperationsRegister {.inline.} =
...
proc isOperationRegisterName*(name: Rune): bool {.inline.} =
  ...
  char(name) >= '0' and char(name) <= '9' or char(name) >= 'A' and char(name) <= 'Z' or
    char(name) >= 'a' and char(name) <= 'z'

proc clearOperations*(r: var Registers, name: Rune): Result[(), string] =
...
proc addOperation*(r: var Registers, name: Rune, operation: Runes): Result[(), string] =
...
proc getOperations*(r: Registers, name: Rune): Result[OperationRegister, string] =
