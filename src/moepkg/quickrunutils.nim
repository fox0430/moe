...
import unicodeext, settings, bufferstatus, gapbuffer, fileutils, backgroundprocess
...
proc nimQuickRunCommand(
    path: string, settings: QuickRunSettings
...
proc clangQuickRunCommand(
  ...
  BackgroundProcessCommand(
    cmd: "/bin/bash", args: @["-c", fmt"'gcc {settings.clangOptions} {path} && ./.out'"]
  )
...
proc cppQuickRunCommand(
  ...
  BackgroundProcessCommand(
    cmd: "/bin/bash", args: @["-c", fmt"'g++ {settings.cppOptions} {path} && ./.out'"]
  )
...
proc quickRunCommand(
    path: string, lang: SourceLanguage, buffer: seq[Runes], settings: QuickRunSettings
...
proc quickRunBufferExists*(bufStatuses: seq[BufferStatus], path: string): bool =
  ## Return true if already exists a buffer for the quickrun.

  for b in bufStatuses:
    if b.isQuickRunMode and path == $b.path:
      return true

proc quickRunBufferIndex*(bufStatuses: seq[BufferStatus], path: string): Option[int] =
...
proc startBackgroundQuickRun*(
    bufStatus: var BufferStatus, settings: EditorSettings
  ...
  if settings.quickRun.saveBufferWhenQuickRun or useTempFile:
    ...
    let r =
      saveFile(path.toRunes, bufStatus.buffer.toRunes, bufStatus.characterEncoding)
    if r.isErr:
  ...
  let command = quickRunCommand(
    path, bufStatus.language, bufStatus.buffer.toSeqRunes, settings.quickRun
  )
  if command.isErr:
...
proc result*(p: var QuickRunProcess): Result[seq[string], string] =
  ...
  if p.isTempFile:
    ...
    if p.filePath.split(".")[0].fileExists:
      removeFile(p.filePath.split(".")[0])
