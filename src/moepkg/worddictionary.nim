...
proc collect*(wordDictionary: WordDictionary, word: Runes): seq[Runes] =
...
iterator enumerateWords*(text: Runes): Runes =
  for word in split(text, r => r.unicodeCategory notin SucceedingCharacter, true):
...
proc update*(
  ...
  for word in text.enumerateWords:
    if exclude != word and not d.contains(word):
      d.add word
