...
proc findCursorPosition*(view: EditorView, line, column: int): tuple[success: bool, y, x: int] =
  for y in 0..view.height-1:
    if view.originalLine[y] != line: continue
    if view.start[y] <= column and column < view.start[y]+view.length[y]:
      return (true, y, if view.start[y] == column: 0 else: width(view.lines[y][0 .. column-view.start[y]-1]))
    if (y == view.height-1 or view.originalLine[y] != view.originalLine[y+1]) and view.start[y]+view.length[y] == column:
      ...
      cursorY = y
      cursorX = if view.start[y] == column: 0 else: width(view.lines[y][0 .. column-view.start[y]-1])
...
proc updatePosition(cursor: var CursorPosition, view: EditorView, line, column: int) =
  ...
  doAssert(success, fmt"Failed to update cursorPosition: (y, x) = ({line}, {column}), originalLine = {view.originalLine}, start = {view.start}, length = {view.length}, lines = {view.lines}, height = {view.height}, width = {view.width}")
