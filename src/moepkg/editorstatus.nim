...
import
  std/[
    strutils, os, strformat, tables, times, heapqueue, deques, options, encodings, math,
    logging, json,
  ]
...
import
  gapbuffer, editorview, ui, unicodeext, highlight, fileutils, windownode, color,
  settings, statusline, bufferstatus, cursor, tabline, backup, messages, commandline,
...
proc getLastCursorPosition*(
    lastPosition: seq[LastCursorPosition], path: Runes
): Option[LastCursorPosition] =
...
proc cancelLspForegroundRequest*(
    c: var LspClient, bufferId: int
): Result[LspMethod, string] =
  if c.isInitialized:
...
proc cancelLspForegroundRequest*(status: var EditorStatus) =
  if status.lspClients.contains(currentBufStatus.langId) and lspClient.isInitialized and
...
proc initLspExperimentalParams*(
    serverName: string, settings: LspServerSettings
): Option[JsonNode] =
  case serverName
  of "rust-analyzer":
    return some(
      experimentClientCapabilities(
        RustAnalyzerConfigs(
...
proc addNewBuffer*(
  ...
  of Mode.diff:
    ...
    let diffResult =
      initDiffViewerBuffer(sourceFilePath, backupFilePath)
    if diffResult.isOk:
    ...
    for i in 0 ..< queueLength:
      ...
      if node.isActualWin:
        let
          ...
          widthOfLineNum =
            if status.settings.view.lineNumber: node.view.widthOfLineNum else: 0
        ...
        if enableStatusLine and
          ...
          status.statusLine[statusLineIndex].windowIndex = node.windowIndex
          inc(statusLineIndex)
...
proc updateStatusLine(status: var EditorStatus) =
  if not status.settings.statusLine.multipleStatusLine:
    ...
    status.statusLine[0].updateStatusLine(
      status.bufStatus[index], currentMainWindowNode, IsActiveWindow, status.settings
    )
  else:
    for i in 0 ..< status.statusLine.len:
      ...
      status.statusLine[i].updateStatusLine(
        status.bufStatus[bufferIndex], node, isActiveWindow, status.settings
      )
...
proc sendLspSemanticTokenRequest*(c: var LspClient, b: BufferStatus) =
  ...
  block:
    ...
    let err = waitFor c.cancelRequest(b.id, LspMethod.textDocumentSemanticTokensFull)
    if err.isErr:
...
proc sendLspInlayHintRequest*(
    c: var LspClient, b: BufferStatus, bufferIndex: int, mainWindowNode: WindowNode
) =
  ## Send textDocument/inlayHint requests to the LSP server.
  ...
  b.inlayHints.range = Range(first: hintRange.first.line, last: hintRange.last.line)
...
proc sendLspInlineValueRequest*(
  ...
  b.inlineValues.range = Range(first: valueRange.first.line, last: valueRange.last.line)
...
proc getLspCapabilities(
    lspClients: LspClientTable, langId: string
): Option[LspCapabilities] =
  if lspClients.contains(langId) and lspClients[langId].isInitialized:
    ...
    if b.isFilerMode:
      if status.filerStatuses[b.filerStatusIndex.get].isUpdateView:
        ...
        b.highlight = initFilerHighlight(
          status.filerStatuses[b.filerStatusIndex.get], b.buffer, n.currentLine
        )
      ...
      let lang =
        if not status.settings.standard.syntax: SourceLanguage.langNone else: b.language
      b.highlight = initHighlight(
        b.buffer.toSeqRunes, status.settings.highlight.reservedWords, lang
      )

      b.version.inc
      b.isUpdate = false

      if status.settings.git.showChangedLine and b.isTrackingByGit:
        b.isGitUpdate = true

      if status.lspClients.isInitialized(b.langId) and b.isEditMode:
        template client(): LspClient =
          status.lspClients[b.langId]
        ...
        if isSendDidChange():
          ...
          let err =
            waitFor client.textDocumentDidChange(b.version, absPath, b.buffer.toString)
          if err.isErr:
            error fmt"lsp: {err.error}"
...
template updateLogViewerBuffer(b: var BufferStatus) =
  ...
  of lsp:
    if status.lspClients.contains(b.logLspLangId):
      b.buffer =
        initLspLogViewrBuffer(status.lspClients[b.logLspLangId].log).toGapBuffer
...
proc updateSelectedArea(b: var BufferStatus, windowNode: var WindowNode) =
  if b.isVisualLineMode:
    let
      ...
      column =
        if b.buffer[currentLine].high > 0:
          b.buffer[currentLine].high
        else:
          0
...
proc updateCommandLine(status: var EditorStatus) =
  ...
  if currentBufStatus.mode.isEditMode and currentBufStatus.syntaxCheckResults.len > 0:
...
proc updateEditorViewConfig(view: var EditorView, settings: EditorSettings) =
  ...
  template viewConf(): var EditorViewConfig =
    view.config
...
proc shiftFoldingRanges*(status: var EditorStatus, start, shift: int) =
  let nodes = mainWindowNode.searchByBufferIndex(status.bufferIndexInCurrentWindow)
...
proc update*(status: var EditorStatus) =
  ...
  for i in 0 .. status.bufStatus.high:
    ...
    if b.isFilerMode and b.filerStatusIndex.isSome:
      ...
      if status.filerStatuses[filerIndex].isUpdatePathList:
        ...
        status.bufStatus[i].buffer = status.filerStatuses[filerIndex].initFilerBuffer(
          settings.filer.showIcons
        ).toGapBuffer
    elif b.isLogViewerMode:
      ...
      status.bufStatus[i].buffer = status.bufStatus.initDebugModeBuffer(
        status.mainWindow.root, currentMainWindowNode.windowIndex,
    ...
    for i in 0 ..< queue.len:
      ...
      if node.isActualWin:
        ...
        if node.view.isFoldingStartLine(node.currentLine):
          if node.currentColumn > 0:
            node.currentColumn = 0
        elif not b.isInsertMode and not b.isReplaceMode and not b.isConfigMode and
            not b.isVisualMode and b.buffer[node.currentLine].len > 0 and
        ...
        block updateTerminalBuffer:
          ...
          node.view.isCurrentWin =
            if node.windowIndex == currentMainWindowNode.windowIndex: true else: false

          template isOverwriteViewBuffer(b: BufferStatus): bool =
            b.inlayHints.hints.len > 0 or b.codeLenses.len > 0 or
          ...
          if isOverwriteViewBuffer(b):
            ...
            if b.inlayHints.hints.len > 0:
              ...
              for hint in b.inlayHints.hints:
                if hint.textEdits.isSome and hint.textEdits.get.len > 0 and
                  ...
                  highlight.addColorSegment(
                    line, text.len, EditorColorPairIndex.inlayHint
                  )
            ...
            if b.inlineValues.values.len > 0:
              ...
              for val in b.inlineValues.values:
                if val.range.start.line < b.buffer.len:
                  ...
                  highlight.addColorSegment(
                    line, text.len, EditorColorPairIndex.inlineValue
                  )
            ...
            if b.codeLenses.len > 0:
              ...
              for l in b.codeLenses:
                if l.command.isSome:
                  ...
                  if index > -1:
                    ...
                    addTexts.add (
                      line: l.range.start.line, text: l.command.get.title.toRunes
                    )
              ...
              for t in addTexts:
                ...
                highlight.addColorSegment(
                  t.line, t.text.len, EditorColorPairIndex.codeLens
                )
  ...
  if not currentBufStatus.isFilerMode:
    let
      ...
      x =
        currentMainWindowNode.view.sidebarWidth +
        currentMainWindowNode.view.widthOfLineNum + currentMainWindowNode.cursor.x
  ...
  if not currentBufStatus.isCommandLineMode and status.recodingOperationRegister.isSome:
...
proc recordCurrentPosition*(
  ...
  bufStatus.positionRecord[bufStatus.buffer.lastSuitId] =
    (windowNode.currentLine, windowNode.currentColumn, windowNode.expandedColumn)
...
proc smoothScrollUpNumberOfLines(
    status: var EditorStatus, numberOfLines: int
): Option[Rune] =
  ...
  let
    ...
    delays = smoothScrollDelays(
      totalLines, status.settings.smoothScroll.minDelay,
...
proc scrollDownNumberOfLines(status: var EditorStatus, numberOfLines: int) =
  let
    ...
    currentLine = currentMainWindowNode.currentLine
    destination = min(
      currentMainWindowNode.currentLine + numberOfLines, currentBufStatus.buffer.len - 1
    )
  ...
  if not (
    view.originalLine[0] <= destination and (
    ...
    let
      ...
      startOfPrintedLines = max(destination - (currentLine - firstOriginLineInView), 0)
...
proc smoothScrollDownNumberOfLines(
  ...
  for i in currentLine ..< destination:
    if i == currentBufStatus.buffer.high:
      break
...
proc smoothHalfPageDown*(status: var EditorStatus): Option[Rune] {.inline.} =
  status.smoothScrollDownNumberOfLines(int(currentMainWindowNode.view.height / 2))
...
proc autoSave(status: var EditorStatus) =
  ...
  for index, bufStatus in status.bufStatus:
    if bufStatus.isAutosave(interval):
      let r =
        saveFile(bufStatus.path, bufStatus.buffer.toRunes, bufStatus.characterEncoding)
      if r.isErr:
        addMessageLog fmt"Failed to auto save: {bufStatus.path}: {r.get}".toRunes
        continue
      ...
      if bufStatus.isEditMode and status.lspClients.contains(bufStatus.langId):
        ...
        let err = waitFor status.lspClients[bufStatus.langId].textDocumentDidSave(
          bufStatus.version, $bufStatus.path.absolutePath, $bufStatus.buffer
        )
        if err.isErr:
          error fmt"lsp: {err.error}"

      status.commandLine.writeMessageAutoSave(
        bufStatus.path, status.settings.notification
      )
    ...
    if p.isRunning:
      ...
      if r.isOk:
        ...
        status.commandLine.writeMessageSuccessBuildOnSave(
          p.filePath, status.settings.notification
        )
      else:
...
proc updateQuickRunBuffer(bufStatus: var BufferStatus, quickRunResult: seq[string]) =
...
proc runBackgroundTasks*(status: var EditorStatus) =
  ...
  if status.settings.standard.liveReloadOfFile:
    ...
    if 0 == currentBufStatus.countChange and
      ...
      if newTextAndEncoding.isErr:
        addMessageLog fmt"Failed to reload the {currentBufStatus.path}: {newTextAndEncoding.error}".toRunes
      else:
  ...
  block automaticBackups:
    ...
    if status.settings.autoBackup.enable and lastBackupTime + interval.minutes < now() and
    ...
    if status.lastOperatingTime + interval < now():
      for i, buf in status.bufStatus:
        if displayBufIndexes.contains(i) and buf.isGitUpdate:
          ...
          let gitDiffProcess =
            startBackgroundGitDiff(buf.path, buf.buffer.toRunes, buf.characterEncoding)
          if gitDiffProcess.isOk:
            status.backgroundTasks.gitDiff.add gitDiffProcess.get
          else:
            status.commandLine.writeGitInfoUpdateError(gitDiffProcess.error)
