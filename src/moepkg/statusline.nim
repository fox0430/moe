...
import
  ui, bufferstatus, color, unicodeext, settings, windownode, gapbuffer, git, fileutils
...
proc statusLineColor(mode: Mode, isActiveWindow: bool): EditorColorPairIndex =
  ...
  of Mode.insert, Mode.insertMulti:
    if isActiveWindow:
      return EditorColorPairIndex.statusLineInsertMode
    else:
      return EditorColorPairIndex.statusLineInsertModeInactive
  of Mode.visual, Mode.visualBlock, Mode.visualLine:
    if isActiveWindow:
      return EditorColorPairIndex.statusLineVisualMode
    else:
      return EditorColorPairIndex.statusLineVisualModeInactive
  of Mode.replace:
    if isActiveWindow:
      return EditorColorPairIndex.statusLineReplaceMode
    else:
      return EditorColorPairIndex.statusLineReplaceModeInactive
  of Mode.ex:
    if isActiveWindow:
      return EditorColorPairIndex.statusLineExMode
    else:
      return EditorColorPairIndex.statusLineExModeInactive
  of Mode.filer:
    if isActiveWindow:
      return EditorColorPairIndex.statusLineFilerMode
    else:
      return EditorColorPairIndex.statusLineFilerModeInactive
  else:
    if isActiveWindow:
      return EditorColorPairIndex.statusLineNormalMode
    else:
      return EditorColorPairIndex.statusLineNormalModeInactive

proc currentLineNumber(node: WindowNode): int {.inline.} =
  node.currentLine + 1

proc totalLines(b: BufferStatus): int {.inline.} =
  b.buffer.len
...
proc statusLineWidth(s: StatusLine): int {.inline.} =
  s.window.width

proc statusLineInfoBuffer(b: BufferStatus, node: WindowNode, setupText: Runes): Runes =
  ...
  result = result.replace(ru"{columnNumber}", currentColumnNumber(node).toRunes)
  # StatusLineItem.columnInPercent
  result = result.replace(ru"{columnInPercent}", columnInPercent(b, node).toRunes)
...
proc statusLineFilerInfoBuffer(b: BufferStatus, node: WindowNode): Runes {.inline.} =
...
proc addFilerModeInfo(
  ...
  if s.statusLineWidth > s.buffer.len + buffer.len + info.len:
    ...
    buffer.add ru " ".repeat(s.statusLineWidth - s.buffer.len - buffer.len - info.len)
...
proc statusLineBufManagerInfoBuffer(
    b: BufferStatus, node: WindowNode
): Runes {.inline.} =
...
proc addBufManagerModeInfo(
  ...
  if s.statusLineWidth > buffer.len + s.buffer.len + info.len:
    # Add spaces before info.
    buffer.add ' '.repeat(s.statusLineWidth - buffer.len - s.buffer.len - info.len).toRunes
...
proc addNormalModeInfo(
  ...
  let info = statusLineInfoBuffer(bufStatus, windowNode, settings.statusLine.setupText)
...
proc isGitBranchName(
    bufStatus: BufferStatus, isActiveWindow: bool, settings: EditorSettings
): bool {.inline.} =
...
proc gitBranchNameBuffer(branchName: Runes, withGitChangedLine: bool): Runes =
...
proc changedLinesBuffer(changedLines: seq[Diff], withGitBranch: bool): Runes =
...
proc addGitInfo(
  ...
  if isGitBranchName(bufStatus, isActiveWindow, settings):
    ...
    if name.isOk:
      branchName = some(name.get)

  var changedLinesBuffer: Runes
  if isGitChangedLine(bufStatus, isActiveWindow, settings):
    let changedLinesBuffer =
      changedLinesBuffer(bufStatus.changedLines, branchName.isSome)
  ...
  if branchName.isSome:
    let
      ...
      branchNameBuffer = gitBranchNameBuffer(branchName.get, withChangedLine)
...
proc modeLabel(mode: Mode): string =
  case mode
  of Mode.insert: "INSERT"
  of Mode.insertMulti: "INSERT MULTI"
  of Mode.visual: "VISUAL"
  of Mode.visualBlock: "VISUAL BLOCK"
  of Mode.visualLine: "VISUAL LINE"
  of Mode.replace: "REPLACE"
  of Mode.filer: "FILER"
  of Mode.bufManager: "BUFFER"
  of Mode.ex: "EX"
  of Mode.logViewer: "LOG"
  of Mode.recentFile: "RECENT"
  of Mode.quickRun: "QUICKRUN"
  of Mode.backup: "BACKUP"
  of Mode.diff: "DIFF"
  of Mode.config: "CONFIG"
  of Mode.debug: "DEBUG"
  of Mode.references: "REFERENCES"
  of Mode.callhierarchyViewer: "CALL HIERARCHY"
  else: "NORMAL"
...
proc addModeLabel(
  ...
  if modeLabel.isSome:
    let buffer =
      if windowNode.x > 0:
        fmt"  {modeLabel.get} ".toRunes
      else:
        fmt" {modeLabel.get} ".toRunes

    s.highlight.segments.add StatusLineColorSegment(
      first: 0, last: buffer.high, color: modeLabelColor(bufStatus.mode)
    )
    s.buffer.add buffer
...
proc updateStatusLineBuffer(
  ...
  if settings.statusLine.mode:
    s.addModeLabel(bufStatus, windowNode, isActiveWindow, settings.statusLine)
