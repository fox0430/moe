...
type
  Diagnostics* = object
    ...
    diagnostics*: seq[Diagnostic] # Diagnostics results

  LspDiagnosticsResult* = Result[Option[Diagnostics], string]

proc parseTextDocumentPublishDiagnosticsNotify*(n: JsonNode): LspDiagnosticsResult =
  ...
  return LspDiagnosticsResult.ok some(
    Diagnostics(path: path.get, diagnostics: params.diagnostics.get)
  )
