...
import std/[strformat, strutils, json, options, os, posix, tables, times, logging]
...
import
  jsonrpc, utils, completion, progress, hover, semantictoken, inlayhint, definition,
  references, rename, typedefinition, implementation, callhierarchy, documenthighlight,
  documentlink, codelens, foldingrange, executecommand, selectionrange, documentsymbol,
...
type
  ...
  LspClient* = ref object
    ...
    log*: LspLog # Request/Response log.
...
template addRequestLog*(c: LspClient, m: JsonNode) =
  c.log.add LspMessage(timestamp: now(), kind: LspMessageKind.request, message: m)

template addResponseLog*(c: LspClient, m: JsonNode) =
  c.log.add LspMessage(timestamp: now(), kind: LspMessageKind.response, message: m)

template addNotifyFromClientLog*(c: LspClient, m: JsonNode) =
  c.log.add LspMessage(
    timestamp: now(), kind: LspMessageKind.notifyFromClient, message: m
  )

template addNotifyFromServerLog*(c: LspClient, m: JsonNode) =
  c.log.add LspMessage(
    timestamp: now(), kind: LspMessageKind.notifyFromServer, message: m
  )
...
proc beginProgress*(
    c: LspClient, token: ProgressToken, p: WorkDoneProgressBegin
): Result[(), string] =
...
proc reportProgress*(
    c: LspClient, token: ProgressToken, report: WorkDoneProgressReport
): Result[(), string] =
...
proc endProgress*(
    c: LspClient, token: ProgressToken, p: WorkDoneProgressEnd
): Result[(), string] =
...
proc notify(
    c: LspClient, lspMethod: LspMethod, params: JsonNode
...
template isLspError*(res: JsonNode): bool =
  res.contains("error")
...
proc setWaitResponse*(c: LspClient, bufferId: int, lspMethod: LspMethod) {.inline.} =
  c.waitingResponses[c.lastId] =
    WaitLspResponse(bufferId: bufferId, requestId: c.lastId, lspMethod: lspMethod)
...
proc isWaitingResponse*(
    c: LspClient, bufferId: int, lspMethod: LspMethod
): bool {.inline.} =
  for v in c.waitingResponses.values:
    if v.bufferId == bufferId and v.lspMethod == lspMethod:
      return true

proc isWaitingForegroundResponse*(c: LspClient, bufferId: int): bool {.inline.} =
  for v in c.waitingResponses.values:
    if v.bufferId == bufferId and v.lspMethod.isForegroundWait:
      return true

proc getWaitingResponse*(
    c: LspClient, id: RequestId
): Option[WaitLspResponse] {.inline.} =
  if c.waitingResponses.contains(id):
...
proc getForegroundWaitingResponse*(
    c: LspClient, bufferId: int
): Option[WaitLspResponse] =
  for v in c.waitingResponses.values:
    if v.bufferId == bufferId and v.lspMethod.isForegroundWait:
      return some(v)
...
proc initInitializeParams*(
    ...
    trace: TraceValue,
    experimental: Option[JsonNode] = none(JsonNode),
): InitializeParams =
  let
    path =
  ...
  result = InitializeParams(
    ...
    rootUri: uri,
    locale: some("en_US"),
    clientInfo: some(ClientInfo(name: "moe", version: some(moeSemVersionStr()))),
    capabilities: ClientCapabilities(
      workspace: some(
        WorkspaceClientCapabilities(
          applyEdit: some(true),
          didChangeConfiguration:
            some(DidChangeConfigurationCapability(dynamicRegistration: some(true))),
          executeCommand:
            some(ExecuteCommandClientCapability(dynamicRegistration: some(true))),
          codeLens:
            some(CodeLensWorkspaceClientCapabilities(refreshSupport: some(true))),
          semanticTokens:
            some(SemanticTokensWorkspaceClientCapabilities(refreshSupport: some(true))),
          inlayHint:
            some(InlayHintWorkspaceClientCapabilities(refreshSupport: some(true))),
        )
      ),
      textDocument: some(
        TextDocumentClientCapabilities(
          hover: some(
            HoverClientCapabilities(
              dynamicRegistration: some(true), contentFormat: some(@["plaintext"])
            )
          ),
          signatureHelp: some(
          ...
          publishDiagnostics:
            some(PublishDiagnosticsClientCapabilities(dynamicRegistration: some(true))),
          formatting:
            some(DocumentFormattingClientCapabilities(dynamicRegistration: some(true))),
          foldingRange: some(
            FoldingRangeClientCapabilities(
              dynamicRegistration: some(true), lineFoldingOnly: some(true)
            )
          ),
          completion: some(
          ...
          selectionRange:
            some(SelectionRangeClientCapabilities(dynamicRegistration: some(true))),
          semanticTokens: some(
            SemanticTokensClientCapabilities(
              ...
              requests: SemanticTokensClientCapabilitiesRequest(
                range: some(false), full: some(true)
              ),
            )
          ),
          inlayHint: some(
          ...
          inlineValue:
            some(InlineValueClientCapabilitie(dynamicRegistration: some(true))),
          declaration: some(
            DeclarationClientCapabilities(
              dynamicRegistration: some(true), linkSupport: some(false)
            )
          ),
          definition:
            some(DefinitionClientCapabilities(dynamicRegistration: some(true))),
          references: some(ReferenceClientCapabilities(dynamicRegistration: some(true))),
          rename: some(
            RenameClientCapabilities(
              dynamicRegistration: some(true), prepareSupport: some(false)
            )
          ),
          typeDefinition: some(
          ...
          codeLens:
            some(CodeLensClientClientCapabilities(dynamicRegistration: some(true))),
          documentHighlight:
            some(DocumentHighlightClientCapabilies(dynamicRegistration: some(true))),
          documentLink: some(
            DocumentLinkClientCapabilities(
              dynamicRegistration: some(true), toolsopSupport: some(false)
            )
          ),
        )
      ),
      window: some(WindowCapabilities(workDoneProgress: some(false))),
      experimental: experimental,
    ),
    workspaceFolders: some(
      @[WorkspaceFolder(uri: workspaceDir.pathToUri, name: workspaceDir.splitPath.tail)]
    ),
    trace: some($trace),
  )

proc setCapabilities(
    c: LspClient, initResult: InitializeResult, settings: LspFeatureSettings
) =
  ## Set server capabilities to the LspClient from InitializeResult.

  var capabilities = LspCapabilities()

  if settings.completion.enable and initResult.capabilities.completionProvider.isSome:
  ...
  if settings.declaration.enable and initResult.capabilities.declarationProvider.isSome:
    if initResult.capabilities.declarationProvider.get.kind == JBool:
      capabilities.declaration = initResult.capabilities.declarationProvider.get.getBool
    else:
      try:
        discard initResult.capabilities.declarationProvider.get.to(DeclarationOptions)
  ...
  if settings.definition.enable and initResult.capabilities.definitionProvider.isSome:
    if initResult.capabilities.definitionProvider.get.kind == JBool:
      capabilities.definition = initResult.capabilities.definitionProvider.get.getBool
    else:
      try:
        discard initResult.capabilities.definitionProvider.get.to(DefinitionOptions)
  ...
  if settings.typeDefinition.enable and
    ...
    if initResult.capabilities.typeDefinitionProvider.get.kind == JBool:
      ...
      try:
        discard
          initResult.capabilities.typeDefinitionProvider.get.to(TypeDefinitionOptions)
  ...
  if settings.implementation.enable and
    ...
    if initResult.capabilities.implementationProvider.get.kind == JBool:
      ...
      try:
        discard
          initResult.capabilities.implementationProvider.get.to(ImplementationOptions)
  ...
  if settings.diagnostics.enable and initResult.capabilities.diagnosticProvider.isSome:
    try:
  ...
  if settings.signatureHelp.enable and
    ...
    capabilities.signatureHelp = initResult.capabilities.signatureHelpProvider

  if settings.hover.enable and initResult.capabilities.hoverProvider.isSome:
    if initResult.capabilities.hoverProvider.get.kind == JBool:
      capabilities.hover = initResult.capabilities.hoverProvider.get.getBool
    else:
      try:
  ...
  if settings.inlayHint.enable and initResult.capabilities.inlayHintProvider.isSome:
    capabilities.inlayHint = true

  if settings.inlineValue.enable and initResult.capabilities.inlineValueProvider.isSome:
    if initResult.capabilities.inlineValueProvider.get.kind == JBool:
      capabilities.inlineValue = initResult.capabilities.inlineValueProvider.get.getBool
    else:
      try:
        discard initResult.capabilities.inlineValueProvider.get.to(InlineValueOptions)
  ...
  if settings.references.enable and initResult.capabilities.referencesProvider.isSome:
    if initResult.capabilities.referencesProvider.get.kind == JBool:
      capabilities.references = initResult.capabilities.referencesProvider.get.getBool
    else:
      try:
        discard initResult.capabilities.referencesProvider.get.to(ReferenceOptions)
  ...
  if settings.callHierarchy.enable and
    ...
    if initResult.capabilities.callHierarchyProvider.get.kind == JBool:
      ...
      try:
        discard
          initResult.capabilities.callHierarchyProvider.get.to(CallHierarchyOptions)
  ...
  if settings.documentLink.enable and initResult.capabilities.documentLinkProvider.isSome:
    capabilities.documentLink = true

  if settings.codeLens.enable and initResult.capabilities.codeLensProvider.isSome:
    capabilities.codeLens = true

  if settings.rename.enable and initResult.capabilities.renameProvider.isSome:
    if initResult.capabilities.renameProvider.get.kind == JBool:
      capabilities.rename = initResult.capabilities.renameProvider.get.getBool
    else:
      try:
        discard initResult.capabilities.renameProvider.get.to(RenameOptions)
  ...
  if settings.executeCommand.enable and
    ...
    capabilities.executeCommand =
      some(initResult.capabilities.executeCommandProvider.get.commands)

  if settings.foldingRange.enable and initResult.capabilities.foldingRangeProvider.isSome:
    if initResult.capabilities.foldingRangeProvider.get.kind == JBool:
      ...
      try:
        discard initResult.capabilities.foldingRangeProvider.get.to(FoldingRangeOptions)
        capabilities.rename = true
      except CatchableError:
  ...
  if settings.selectionRange.enable and
    ...
    if initResult.capabilities.selectionRangeProvider.get.kind == JBool:
      ...
      try:
        discard
          initResult.capabilities.selectionRangeProvider.get.to(SelectionRangeOptions)
  ...
  if settings.documentSymbol.enable and
    ...
    if initResult.capabilities.documentSymbolProvider.get.kind == JBool:
      ...
      try:
        discard
          initResult.capabilities.documentSymbolProvider.get.to(DocumentSymbolOptions)
...
proc cancelRequest*(
    c: LspClient, bufferId: int, requestId: RequestId
): Future[LspSendNotifyResult] {.async.} =
...
proc cancelRequest*(
    c: LspClient, waitRes: WaitLspResponse
): Future[LspSendRequestResult] {.async.} =
...
proc initialize*(
    c: LspClient, bufferId: int, initParams: InitializeParams
): Future[LspSendRequestResult] {.async.} =
...
proc initCapacities*(
    c: LspClient, settings: LspFeatureSettings, res: JsonNode
): LspInitializeResult =
...
): DidOpenTextDocumentParams {.inline.} =
  DidOpenTextDocumentParams(
    textDocument:
      TextDocumentItem(uri: uri, languageId: languageId, version: 1, text: text)
  )

proc textDocumentDidOpen*(
    c: LspClient, path, languageId, text: string
): Future[LspSendNotifyResult] {.async.} =
  ...
  let params = %*initTextDocumentDidOpenParams(path.pathToUri, languageId, text)

  let err = await c.notify(LspMethod.textDocumentDidOpen, params)
  if err.isErr:
    return
      LspSendNotifyResult.err fmt"textDocument/didOpen notification failed: {err.error}"

  return LspSendNotifyResult.ok ()

proc initTextDocumentDidChangeParams(
    version: Natural, path, text: string, range: Option[BufferRange] = none(BufferRange)
): DidChangeTextDocumentParams {.inline.} =
  if range.isSome:
    ## Send range
    return DidChangeTextDocumentParams(
      textDocument:
        VersionedTextDocumentIdentifier(uri: path.pathToUri, version: some(%version)),
      contentChanges:
        @[TextDocumentContentChangeEvent(text: text, range: some(range.get.toLspRange))],
    )
  else:
    ## Send all text
    return DidChangeTextDocumentParams(
...
proc textDocumentDidChange*(
    ...
    version: Natural,
    path, text: string,
    range: Option[BufferRange] = none(BufferRange),
): Future[LspSendNotifyResult] {.async.} =
...
proc initTextDocumentDidSaveParams(
    version: Natural, path, text: string
): DidSaveTextDocumentParams {.inline.} =
...
proc textDocumentDidSave*(
    c: LspClient, version: Natural, path, text: string
): Future[LspSendNotifyResult] {.async.} =
...
proc initTextDocumentDidClose(path: string): DidCloseTextDocumentParams {.inline.} =
  DidCloseTextDocumentParams(textDocument: TextDocumentIdentifier(uri: path.pathToUri))

proc textDocumentDidClose*(
    c: LspClient, text: string
): Future[LspSendNotifyResult] {.async.} =
...
proc textDocumentHover*(
    c: LspClient, bufferId: int, path: string, position: BufferPosition
): Future[LspSendRequestResult] {.async.} =
...
proc textDocumentSemanticTokens*(
    c: LspClient, bufferId: int, path: string
): Future[LspSendRequestResult] {.async.} =
...
proc textDocumentInlayHint*(
    c: LspClient, bufferId: int, path: string, range: BufferRange
): Future[LspSendRequestResult] {.async.} =
...
proc textDocumentDefinition*(
    c: LspClient, bufferId: int, path: string, posi: BufferPosition
): Future[LspSendRequestResult] {.async.} =
...
proc textDocumentRename*(
    c: LspClient, bufferId: int, path: string, posi: BufferPosition, newName: string
): Future[LspSendRequestResult] {.async.} =
...
proc textDocumentIncomingCalls*(
    c: LspClient, bufferId: int, item: CallHierarchyItem
): Future[LspSendRequestResult] {.async.} =
...
proc documentLinkResolve*(
    c: LspClient, bufferId: int, documentLink: DocumentLink
): Future[LspSendRequestResult] {.async.} =
...
proc codeLensResolve*(
    c: LspClient, bufferId: int, codeLens: CodeLens
): Future[LspSendRequestResult] {.async.} =
...
proc workspaceExecuteCommand*(
    c: LspClient, bufferId: int, command: string, args: JsonNode
): Future[LspSendRequestResult] {.async.} =
...
proc textDocumentSelectionRange*(
    c: LspClient, bufferId: int, path: string, positions: seq[BufferPosition]
): Future[LspSendRequestResult] {.async.} =
...
proc textDocumentFormatting*(
    c: LspClient, bufferId: int, path: string, options: FormattingOptions
): Future[LspSendRequestResult] {.async.} =
