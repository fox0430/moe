...
proc parseWindowWorkDnoneProgressCreateNotify*(
    n: JsonNode
): LspWindowWorkDnoneProgressCreateResult =
  ...
  if params.token.isNone:
    return
      LspWindowWorkDnoneProgressCreateResult.err fmt"Invalid notify: token is empty"
...
template isProgressNotify(j: JsonNode): bool =
  ...
  j.contains("params") and j["params"].kind == JObject and j["params"].contains("token") and
    j["params"].contains("value") and j["params"]["value"].contains("kind") and
    j["params"]["value"]["kind"].kind == JString

template isWorkDoneProgressBegin*(j: JsonNode): bool =
  isProgressNotify(j) and "begin" == j["params"]["value"]["kind"].getStr

template isWorkDoneProgressReport*(j: JsonNode): bool =
  isProgressNotify(j) and "report" == j["params"]["value"]["kind"].getStr

template isWorkDoneProgressEnd*(j: JsonNode): bool =
  isProgressNotify(j) and "end" == j["params"]["value"]["kind"].getStr

template workDoneProgressToken*(j: JsonNode): string =
  j["params"]["token"].getStr

proc parseWorkDoneProgressBegin*(n: JsonNode): LspWorkDoneProgressBeginResult =
  ...
  try:
    return
      LspWorkDoneProgressBeginResult.ok n["params"]["value"].to(WorkDoneProgressBegin)
  except CatchableError as e:
    return LspWorkDoneProgressBeginResult.err fmt"Invalid notify: {e.msg}"

proc parseWorkDoneProgressReport*(n: JsonNode): LspWorkDoneProgressReportResult =
  ## https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#progress
  ...
  try:
    return
      LspWorkDoneProgressReportResult.ok n["params"]["value"].to(WorkDoneProgressReport)
  except CatchableError as e:
    return LspWorkDoneProgressReportResult.err fmt"Invalid notify: {e.msg}"

proc parseWorkDoneProgressEnd*(n: JsonNode): LspWorkDoneProgressEndResult =
  ## https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#progress
  ...
  try:
    return LspWorkDoneProgressEndResult.ok n["params"]["value"].to(WorkDoneProgressEnd)
  except CatchableError as e:
    return LspWorkDoneProgressEndResult.err fmt"Invalid notify: {e.msg}"

template isCancellable*(p: WorkDoneProgressBegin | WorkDoneProgressReport): bool =
