...
type
  ...
  WaitType* = enum
    foreground # foreground request is canceled when a user takes action.
    background
...
proc parseShutdownResponse*(res: JsonNode): LspShutdownResult =
  if res["result"].kind == JNull:
    ...
    return LspShutdownResult.err fmt"Shutdown request failed: {res}"

proc lspMethod*(j: JsonNode): LspMethodResult =
  if not j.contains("method"):
    return LspMethodResult.err "Invalid value"
...
proc isLspResponse*(res: JsonNode): bool {.inline.} =
  res.contains("id")

proc getWaitingType*(lspMethod: LspMethod): Option[WaitType] =
  case lspMethod
  of initialize:
    some(WaitType.background)
  of shutdown:
    some(WaitType.background)
  of textDocumentHover:
    some(WaitType.foreground)
  of textDocumentCompletion:
    some(WaitType.foreground)
  of textDocumentSemanticTokensFull:
    some(WaitType.background)
  of textDocumentSemanticTokensDelta:
    some(WaitType.background)
  of textDocumentInlayHint:
    some(WaitType.background)
  of textDocumentDefinition:
    some(WaitType.foreground)
  of textDocumentReferences:
    some(WaitType.foreground)
  of textDocumentRename:
    some(WaitType.foreground)
  of textDocumentTypeDefinition:
    some(WaitType.foreground)
  of textDocumentImplementation:
    some(WaitType.foreground)
  of textDocumentDeclaration:
    some(WaitType.foreground)
  of textDocumentPrepareCallHierarchy:
    some(WaitType.foreground)
  of callHierarchyIncomingCalls:
    some(WaitType.foreground)
  of callHierarchyOutgoingCalls:
    some(WaitType.foreground)
  of textDocumentDocumentLink:
    some(WaitType.foreground)
  of documentLinkResolve:
    some(WaitType.foreground)
  of textDocumentCodeLens:
    some(WaitType.foreground)
  of codeLensResolve:
    some(WaitType.foreground)
  of workspaceExecuteCommand:
    some(WaitType.foreground)
  of textDocumentFoldingRange:
    some(WaitType.foreground)
  of textDocumentSelectionRange:
    some(WaitType.foreground)
  of textDocumentDocumentSymbol:
    some(WaitType.foreground)
  of textDocumentFormatting:
    some(WaitType.foreground)
  else:
    none(WaitType)
