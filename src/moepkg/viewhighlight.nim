...
import lsp/client
import
  independentutils, bufferstatus, highlight, color, windownode, gapbuffer, unicodeext,
  editorview, searchutils, settings, ui, syntaxcheck, git, theme
...
proc initBufferPosition(n: WindowNode): BufferPosition {.inline.} =
  BufferPosition(line: n.currentLine, column: n.currentColumn)

proc initBufferInView(bufStatus: BufferStatus, windowNode: WindowNode): BufferInView =
  ...
  let
    ...
    lastLine =
      if range.last > bufStatus.buffer.high: bufStatus.buffer.high else: range.last
...
proc originalPosition(b: BufferInView, line, column: int): BufferPosition {.inline.} =
  BufferPosition(line: b.originalLine(line), column: column)

proc originalPosition(
    b: BufferInView, position: BufferPosition
): BufferPosition {.inline.} =
  b.originalPosition(position.line, position.column)

proc highlightSelectedArea(
    highlight: var Highlight, bufStatus: BufferStatus, position: BufferPosition
) =
  let area = bufStatus.selectedArea.get

  var colorSegment =
    initSelectedAreaColorSegment(position, EditorColorPairIndex.selectArea)
  ...
  if bufStatus.isVisualBlockMode:
    highlight.overwriteColorSegmentBlock(bufStatus.selectedArea.get, bufStatus.buffer)
  elif bufStatus.isVisualMode:
    highlight.overwrite(colorSegment)

proc highlightPairOfParen(highlight: var Highlight, bufferInView: BufferInView) =
  ## Add a highlight for a parenthesis corresponding to the parenthesis on
  ## the cursor.

  template currentLine(): int =
    bufferInView.currentLineInView

  template currentColumn(): int =
    bufferInView.currentPosition.column

  template currentLineBuffer(): Runes =
    bufferInView.buffer[currentLine]

  if currentColumn > currentLineBuffer.high:
    return
...
proc highlightReferences(
    highlight: var Highlight, bufferInView: BufferInView, colorMode: ColorMode
) =
  ...
  template isHighlightWord(line, word: Runes, position: int): bool =
    template beforeRune(): Rune =
      line[position - 1]

    template nextRune(): Rune =
      line[position + word.high + 1]
  ...
  let
    ...
    highlightWord = currentWord(bufferInView.buffer, currentPositionInView)

  if highlightWord.word.len == 0:
    # Empty line or ignore character.
    return

  let highlightWordPositionInView =
    BufferPosition(line: currentPositionInView.line, column: highlightWord.position)

  let results = searchAllOccurrence(bufferInView.buffer, highlightWord.word)
  for position in results:
    template line(): Runes =
      bufferInView.buffer[position.line]
    ...
    if isHighlightWord(line, highlightWord.word, position.column) and
      ...
      let
        ...
        originalEditorColorPairIndex =
          highlight.getColorPair(originalPosition.line, originalPosition.column)
        originalColorPair = themeColors[originalEditorColorPairIndex]
      discard EditorColorPairIndex.currentWord.initColorPair(
        colorMode,
        originalColorPair.foreground,
        themeColors[EditorColorPairIndex.currentWord].background,
      )
...
proc highlightDocumentHighlights(
    h: var Highlight, currentPosition: BufferPosition, ranges: seq[BufferRange]
) =
  ## LSP textDocument/documentHighlight

  template isOnCursor(currentWordPosition: BufferPosition, range: BufferRange): bool =
...
proc highlightFullWidthSpace(
    highlight: var Highlight, windowNode: WindowNode, bufferInView: BufferInView
) =
...
proc highlightText*(
  ...
  let
    positions = bufferInView.buffer.search(
      highlightingText.text, highlightingText.isIgnorecase, highlightingText.isSmartcase
    )
  ...
  for p in positions:
    let
      lasRow =
        bufferInView.originalLineRange.first + p.line + highlightingText.text.high
...
proc highlightSyntaxCheckerReuslts(
    highlight: var Highlight, bufferInView: BufferInView, syntaxErrors: seq[SyntaxError]
) =
...
proc highlightGitConflicts(highlight: var Highlight, bufferInView: BufferInView) =
  ## Add highlights for git conflict marker lines.

  proc isHighlightLine(line: Runes): bool {.inline.} =
    line.isGitConflictStartMarker or line.isGitConflictDivideMarker or
