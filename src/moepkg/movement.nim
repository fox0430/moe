...
import
  editorview, gapbuffer, unicodeext, windownode, bufferstatus, independentutils,
  searchutils

template currentLineLen(): int =
...
proc isExpandPosition*(
  ...
  windowNode.currentColumn == bufStatus.buffer[windowNode.currentLine].high + 1
...
proc keyRight*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  ...
  if windowNode.currentColumn + 1 >= maxColumn:
    return
...
proc keyUp*(
    bufStatus: var BufferStatus, windowNode: var WindowNode, firstLine: int = 0
) =
  if windowNode.currentLine == firstLine:
    return
...
proc keyDown*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  if windowNode.currentLine + 1 == bufStatus.buffer.len:
    return
...
proc getLastNonBlankOfLine*(bufStatus: BufferStatus, windowNode: WindowNode): int =
...
proc moveToFirstNonBlankOfLine*(
  ...
  windowNode.currentColumn = bufStatus.getFirstNonBlankOfLineOrLastColumn(windowNode)
  windowNode.expandedColumn = windowNode.currentColumn

  windowNode.removeFoldingRange

proc moveToLastNonBlankOfLine*(bufStatus: BufferStatus, windowNode: var WindowNode) =
...
proc jumpLine*(bufStatus: BufferStatus, windowNode: var WindowNode, destination: int) =
  ...
  if not (
    ...
    if destination > bufStatus.buffer.high - windowNode.getHeight - 1:
      startOfPrintedLines = max(bufStatus.buffer.high - windowNode.getHeight - 1, 0)
    else:
      startOfPrintedLines =
        max(destination - (currentLine - windowNode.view.originalLine[0]), 0)
...
proc moveToNextBlankLine*(bufStatus: BufferStatus, windowNode: var WindowNode) =
  ...
  if windowNode.findFoldingRange.isNone:
    windowNode.currentColumn = max(bufStatus.buffer[windowNode.currentLine].high, 0)

proc moveToPreviousBlankLine*(bufStatus: BufferStatus, windowNode: var WindowNode) =
...
proc moveToFirstLine*(
    bufStatus: BufferStatus, windowNode: var WindowNode, destination: int = 0
) {.inline.} =
...
proc moveToLastLine*(bufStatus: BufferStatus, windowNode: var WindowNode) =
  if bufStatus.cmdLoop > 1:
    ...
    bufStatus.jumpLine(windowNode, bufStatus.buffer.high)

proc moveToFirstOfWord*(windowNode: var WindowNode, bufStatus: BufferStatus) =
  ## Move to the first position of the current word.

  windowNode.currentColumn = bufStatus.buffer[windowNode.currentLine].findFirstOfWord(
...
proc moveToForwardWord*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  ...
  while true:
    ...
    if isPunct(curr) or isAlpha(curr) or isDigit(curr):
      break
    inc(windowNode.currentColumn)
...
proc moveToBackwardWord*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  if bufStatus.buffer.isFirst(windowNode.currentLine, windowNode.currentColumn):
    return
  ...
  while true:
    ...
    if currentLineLen == 0 or bufStatus.buffer.isFirst(currentLine, currentColumn):
      break

    let curr = bufStatus.buffer[currentLine][currentColumn]
    if unicodeext.isSpace(curr):
      continue
...
proc moveToForwardEndOfWord*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  ...
  if isSkipped == nil:
    ...
    while true:
      inc(windowNode.currentColumn)
      if windowNode.currentColumn == currentLineLen - 1:
        break
...
proc moveToCenterOfScreen*(bufStatus: BufferStatus, windowNode: var WindowNode) =
  ...
  if (bufStatus.buffer.high - windowNode.currentLine) < windowNode.view.height - 1:
    ...
    let medInVisible =
      int((bufStatus.buffer.high - windowNode.view.originalLine[0]) / 2)
...
proc moveToFirstWordOfPrevLine*(windowNode: var WindowNode, bufStatus: BufferStatus) =
  ## Move to the first word of the previous line.
  ## Ignore empty lines and whitespaces.

  if windowNode.bufferPosition.line == 0:
    return
...
proc moveToFirstWordOfNextLine*(windowNode: var WindowNode, bufStatus: BufferStatus) =
  ## Move to the first word of the next line.
  ## Ignore empty lines and whitespaces.

  if windowNode.bufferPosition.line == bufStatus.buffer.high:
    return
...
proc scrollScreenTop*(
  ...
  windowNode.view.reload(
    bufStatus.buffer, windowNode.view.originalLine[windowNode.cursor.y]
  )

proc scrollScreenCenter*(bufStatus: var BufferStatus, windowNode: var WindowNode) =
  if windowNode.currentLine > int(windowNode.view.height / 2):
    if windowNode.cursor.y > int(windowNode.view.height / 2):
      ...
      windowNode.view.reload(
        bufStatus.buffer, windowNode.view.originalLine[startOfPrintedLines]
      )
    else:
      let numOfTime = int(windowNode.view.height / 2) - windowNode.cursor.y
      for i in 0 ..< numOfTime:
        scrollUp(windowNode.view, bufStatus.buffer)
...
proc scrollScreenBottom*(bufStatus: var BufferStatus, windowNode: WindowNode) =
  if windowNode.currentLine > windowNode.view.height:
    ...
    for i in 0 ..< numOfTime:
      windowNode.view.scrollUp(bufStatus.buffer)

proc moveToPairOfParen*(bufStatus: BufferStatus, windowNode: var WindowNode) =
  ## Move to matching pair of paren. Do nothing If no matching pair exists.

  if bufStatus.isExpandPosition(windowNode):
    return
...
proc jumpToSearchForwardResults*(
  ...
  if lines.len == 1:
    searchResult = bufStatus.searchBuffer(
      windowNode.bufferPosition, keyword, isIgnorecase, isSmartcase
    )
  else:
    searchResult = bufStatus.searchBuffer(
      windowNode.bufferPosition, lines, isIgnorecase, isSmartcase
    )
