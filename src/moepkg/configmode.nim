...
proc getStandardTableSettingValues(settings: EditorSettings, name: string): seq[Runes] =
...
proc getClipboardTableSettingsValues(
    settings: ClipboardSettings, name: string
): seq[Runes] =
  case name
  of "enable":
...
proc getBuildOnSaveTableSettingValues(
    settings: BuildOnSaveSettings, name: string
): seq[Runes] =
  case name
  of "enable":
    let currentVal = settings.enable
...
proc getTabLineTableSettingValues(settings: TabLineSettings, name: string): seq[Runes] =
  var currentVal: bool
  case name
  of "allBuffer":
...
proc getStatusLineTableSettingValues(
    settings: StatusLineSettings, name: string
): seq[Runes] =
  if name == "setupText":
...
proc getHighlightTableSettingValues(
    settings: EditorSettings, name: string
): seq[Runes] =
  var currentVal: bool
  case name
  of "currentLine":
...
proc getAutoBackupTableSettingValues(
    settings: AutoBackupSettings, name: string, settingType: SettingType
): seq[Runes] =
  case name
  of "enable":
    let currentVal = settings.enable
...
proc getQuickRunTableSettingValues(
    settings: QuickRunSettings, name: string, settingType: SettingType
): seq[Runes] =
  case name
...
proc getNotificationTableSettingValues(
    settings: NotificationSettings, name: string
): seq[Runes] =
  var currentVal: bool
  case name
...
proc getFilerTableSettingValues(settings: FilerSettings, name: string): seq[Runes] =
  var currentVal: bool
  case name
  of "showIcons":
...
proc getAutocompleteTableSettingValues(
    settings: AutocompleteSettings, name: string
): seq[Runes] =
  var currentVal: bool
  case name
  of "enable":
...
proc getAutoSaveTableSettingValues(
    settings: AutoSaveSettings, name: string
): seq[Runes] =
  case name
  of "enable":
    if settings.enable:
...
proc getPersistTableSettingsValues(
    settings: PersistSettings, name: string
): seq[Runes] =
  case name
...
proc getSyntaxCheckerTableSettingsValues(
    s: SyntaxCheckerSettings, name: string
): seq[Runes] =
  case name
  of "enable":
    if s.enable:
...
proc getSmoothScrollTableSettingsValues(
    s: SmoothScrollSettings, name: string
): seq[Runes] =
  case name
  of "enable":
    if s.enable:
...
proc getStartUpFileOpenSettingsValues(
    s: StartUpFileOpenSettings, name: string
): seq[Runes] =
  case name
  of "autoSplit":
    if s.autoSplit:
...
proc getThemeSettingsValues(s: ThemeSettings, name: string): seq[Runes] =
  ...
  of "kind":
    ...
    for k in ColorThemeKind:
      if $k != name:
        result.add toRunes($k)
  of "path":
    return @[s.path].toSeqRunes

proc getSettingValues(
    settings: EditorSettings, settingType: SettingType, table, name, position: string
): seq[Runes] =
  case table
  of "Standard":
...
proc initConfigModeHighlight[T](
    buffer: T, currentLine: int, reservedWords: seq[ReservedWord]
): Highlight =
  result = Highlight()
...
proc changeStandardTableSetting(
    settings: var EditorSettings, settingName, settingVal: string
) =
  case settingName
  of "number":
...
proc changeClipBoardTableSettings(
    settings: var ClipboardSettings, settingName, settingVal: string
) =
  case settingName
  of "enable":
...
proc changeBuildOnSaveTableSetting(
    settings: var BuildOnSaveSettings, settingName, settingVal: string
) =
  case settingName
  of "enable":
    settings.enable = parseBool(settingVal)
...
proc changeTabLineTableSetting(
    settings: var TabLineSettings, settingName, settingVal: string
) =
  case settingName
  of "allBuffer":
...
proc changeStatusLineTableSetting(
    settings: var StatusLineSettings, settingName, settingVal: string
) =
  case settingName
  of "multipleStatusLine":
...
proc changeBackupTableSetting(
    settings: var AutoBackupSettings, settingName, settingVal: string
) =
  case settingName
  of "enable":
    settings.enable = parseBool(settingVal)
...
proc changeQuickRunTableSetting(
    settings: var QuickRunSettings, settingName, settingVal: string
) =
  case settingName
  of "saveBufferWhenQuickRun":
...
proc changeNotificationTableSetting(
    settings: var NotificationSettings, settingName, settingVal: string
) =
  case settingName
  of "screenNotifications":
...
proc changeAutoCompleteTableSetting(
    settings: var AutocompleteSettings, settingName, settingVal: string
) =
  case settingName
  of "enable":
    settings.enable = parseBool(settingVal)
...
proc changeAutoSaveTableSetting(
    settings: var AutoSaveSettings, settingName, settingVal: string
) =
  case settingName
  of "enable":
    settings.enable = parseBool(settingVal)
...
proc changePerSistTableSettings(
    settings: var PersistSettings, settingName, settingVal: string
) =
  case settingName
  of "exCommand":
...
proc changeGitTableSettings(s: var GitSettings, settingName, settingVal: string) =
  case settingName
  of "showChangedLine":
...
proc changeSyntaxCheckerTableSettings(
    s: var SyntaxCheckerSettings, settingName, settingVal: string
) =
  case settingName
  of "enable":
...
proc changeSmoothScrollTableSettings(
    s: var SmoothScrollSettings, settingName, settingVal: string
) =
  case settingName
  of "enable":
    s.enable = settingVal.parseBool
...
proc changeStartUpFileOpenSettingsTableSettings(
    s: var StartUpFileOpenSettings, settingName, settingVal: string
) =
  case settingName
  of "autoSplit":
...
proc changeThemeTableSetting(s: var ThemeSettings, settingName, settingVal: string) =
  case settingName
  of "kind":
    s.kind = parseEnum[ColorThemeKind](settingVal)

proc changeEditorSettings(
    status: var EditorStatus, table, settingName, settingVal: string
) =
  template settings(): var EditorSettings =
    status.settings
  ...
  of "Notification":
    notificationSettings.changeNotificationTableSetting(settingName, settingVal)
  of "Filer":
...
proc getSettingType(table, name: string): SettingType =
  template standardTable() =
    case name
    of "theme", "defaultCursor", "normalModeCursor", "insertModeCursor":
  ...
  template statusLineTable() =
    case name
    of "multipleStatusLine", "merge", "mode", "filename", "chanedMark", "line",
        "column", "encoding", "language", "directory", "gitChangedLines",
        "gitBranchName", "showGitInactive", "showModeInactive":
      result = SettingType.bool
    of "setupText":
      result = SettingType.text
    else:
  ...
  template highlightTable() =
    case name
    of "currentLine", "fullWidthSpace", "trailingSpaces", "currentWord", "replaceText",
        "pairOfParen":
      result = SettingType.bool
    of "reservedWords":
      result = SettingType.text
  ...
  template quickRunTable() =
    ...
    of "timeout":
      result = SettingType.number
    of "nimAdvancedCommand", "ClangOptions", "CppOptions", "NimOptions", "shOptions",
        "bashOptions":
      result = SettingType.text
    else:
      result = SettingType.none
...
proc insertCharacter(bufStatus: var BufferStatus, windowNode: WindowNode, c: Rune) =
    ...
    if isResizeKey(key):
      ...
      if currentMainWindowNode.currentColumn > minColumn:
        currentBufStatus.keyBackspace(
          currentMainWindowNode, autoDeleteParen, status.settings.standard.tabStop
        )
    else:
      numStr &= key
      ...
      currentBufStatus.highlight =
        currentBufStatus.buffer.initConfigModeHighlight(currentLine, reservedWords)
...
proc editTextSetting(status: var EditorStatus, table, name: string) =
  ...
  while not isBreak and not isCancel:
    ...
    if isResizeKey(key):
      ...
      currentBufStatus.insertCharacter(currentMainWindowNode, key)
      let reservedWords = status.settings.highlight.reservedWords
      currentBufStatus.highlight = currentBufStatus.buffer.initConfigModeHighlight(
        currentMainWindowNode.currentLine, reservedWords
      )
  ...
  if isCancel:
    ...
    template highlightTable() =
      case name
      of "reservedWords":
        status.settings.highlight.reservedWords = buffer.splitWhitespace.mapIt(
...
proc editEnumAndBoolSettings(
  ...
  while settingValues.len > 1:
    ...
    if isTabKey(key) or isDownKey(key) or key == ord('j'):
      if suggestIndex == settingValues.high:
        suggestIndex = 0
      else:
        suggestIndex.inc
...
proc selectAndChangeEditorSettings(status: var EditorStatus, arrayIndex: int) =
  ...
  let
    selectedTable =
      getTableName(currentBufStatus.buffer, currentMainWindowNode.currentLine)
    ...
    settingValues = getSettingValues(
      status.settings, settingType, selectedTable, selectedSetting, position
    )
  ...
  for name in AutoBackupTableNames:
    ...
    of "dirToExclude":
      ...
      for i, d in settings.dirToExclude:
        dirs &= $d
        if i < settings.dirToExclude.high:
          dirs &= ' '
...
proc initNotificationTableBuffer(settings: NotificationSettings): seq[Runes] =
...
proc initStartUpFileOpenTableBuffer(settings: StartUpFileOpenSettings): seq[Runes] =
...
proc initThemeColorsTableBuffer*(s: EditorSettings): seq[Runes] =
  ...
  for pairIndex in EditorColorPairIndex:
    let
      ...
      fgColorText = if fgHex.isSome: fgHex.get else: "termDefaultFg"

      bgColorText = if bgHex.isSome: bgHex.get else: "termDefaultBg"
...
proc changeModeToSearchBackwardMode(
  ...
  commandLine.clear
  commandLine.setPrompt(CommandLinePrompt.searchBackward)
...
proc isConfigModeCommand*(command: Runes): InputState =
  ...
  if command.len == 1:
    let key = command[0]
    if isCtrlK(key) or isCtrlJ(key) or key == ord(':') or key == ord('h') or
        isLeftKey(key) or key == ord('l') or isRightKey(key) or isEnterKey(key) or
        isCtrlU(key) or isCtrlD(key) or isPageUpKey(key) or isPageDownKey(key) or
      ## Page down and Ctrl - F
    key == ord('k') or isUpKey(key) or key == ord('j') or isDownKey(key) or
        key == ord('G') or key == ord('/') or key == ord('?'):
