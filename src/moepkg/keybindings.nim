...
type
  ...
  NormalModeKeyBindings* = TableRef[Keys, seq[NormlaModeCommand]]
...
proc isTargetKey*(command: NormlaModeCommand): bool =
  command in [
    changeForward,
    changeUntil,
    replaceCharacter,
...
proc find*(b: NormalModeKeyBindings, keys: Keys): seq[NormlaModeCommand] =
  for k, v in b.pairs:
    for c in v:
      if keys.startsWith(k): return v
