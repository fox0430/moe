...
proc compare*(rune, sub: Runes, isIgnorecase, isSmartcase: bool): bool {.inline.} =
  ## Return true If the text matches.

  if isIgnorecase and not isSmartcase:
    if cmpIgnoreCase($rune, $sub) == 0:
      return true
...
proc search*(buffer, keyword: Runes, isIgnorecase, isSmartcase: bool): Option[int] =
  ## Return a position if keyword matches.

  for position in 0 .. buffer.high - keyword.high:
    if compare(
      buffer[position .. position + keyword.high], keyword, isIgnorecase, isSmartcase
    ):
      return some(position)

proc searchAll*(buffer, keyword: Runes, isIgnorecase, isSmartcase: bool): seq[int] =
...
proc search*(
    buffer, keyword: seq[Runes], isIgnorecase, isSmartcase: bool
): seq[BufferPosition] =
  ...
  if keyword.len == 0 or (keyword.len == 1 and keyword[0].len == 0):
    return

  let
    bufferRunes = buffer.toRunes

    positionsResult = bufferRunes.searchAll(keyword.toRunes, isIgnorecase, isSmartcase)
    ...
    for i in 0 .. bufferRunes.high:
      ...
      if i == positionsResult[countPositionsResult]:
        if newLinePositions.len == 0:
          ...
          result.add BufferPosition(line: newLinePositions.high, column: col)
        else:
          result.add BufferPosition(
            line: newLinePositions.len, column: i - newLinePositions[^1] - 1
          )

        if countPositionsResult == positionsResult.high:
          break
        else:
          countPositionsResult.inc

proc searchReversely(
    line, keyword: Runes, isIgnorecase, isSmartcase: bool
): Option[int] =
...
proc searchBuffer*(
  ...
  for i in 0 ..< bufStatus.buffer.len:
    let
      ...
      first = if lineNumber == startLine and i == 0: currentPosition.column else: 0
...
proc searchBuffer*(
  ...
  for i in 0 .. bufStatus.buffer.high - keyword.high:
    ...
    if position.len > 0:
      return SearchResult(line: lineNumber, column: position[0].column).some
...
proc searchBufferReversely*(
  ...
  for i in 0 ..< bufStatus.buffer.len + 1:
    ...
    if bufStatus.buffer[lineNumber].len == 0:
      continue
    ...
    let endPosition =
      if lineNumber == startLine:
        ...
        bufStatus.buffer[lineNumber].high

    let position = searchReversely(
      bufStatus.buffer[lineNumber][0 .. endPosition], keyword, isIgnorecase, isSmartcase
    )
    if position.isSome:
...
proc searchBufferReversely*(
  ...
  for i in 0 ..< bufStatus.buffer.len + 1:
    ...
    if lineNumber < 0:
      lineNumber = bufStatus.buffer.len - i
...
proc searchAllOccurrence*(
    buffer: seq[Runes], keyword: Runes, isIgnorecase, isSmartcase: bool
): seq[SearchResult] =
  ## Return a buffer position if a keyword matches.
  ...
  for lineNumber in 0 ..< buffer.len:
    ...
    while first < buffer[lineNumber].len:
      let
        ...
        position = search(line[first ..< last], keyword, isIgnorecase, isSmartcase)
...
proc searchAllOccurrence*(
    buffer: seq[Runes], keyword: Runes
): seq[SearchResult] {.inline.} =
  ## Return a buffer position if a keyword matches.

  searchAllOccurrence(buffer, keyword, false, false)

proc saveSearchHistory*(searchHistory: var seq[Runes], keyword: Runes, limit: int) =
  ## Save a keyword to the searchHistory.
  ## If the size exceeds the limit, the oldest will be deleted.

  if limit < 1 or keyword.len == 0:
    return
...
proc searchClosingParen*(
    buffer: seq[Runes], openParenPosition: BufferPosition
): Option[SearchResult] =
  ...
  if (openParen == ru '"') or (openParen == ru '\''):
    return
  ...
  for i in openParenPosition.line .. buffer.high:
    if buffer[i].len > 0:
      let
        startColumn = if i == openParenPosition.line: openParenPosition.column else: 0
...
proc searchOpeningParen*(
    buffer: seq[Runes], closeParenPosition: BufferPosition
): Option[SearchResult] =
  ...
  if (closeParen == ru '"') or (closeParen == ru '\''):
    return
  ...
  for i in countdown(closeParenPosition.line, 0):
    if buffer[i].len > 0:
      let
        startColumn = 0
        endColumn =
          if i == closeParenPosition.line:
            closeParenPosition.column
          else:
            buffer[i].high
...
proc searchClosingParen(
    bufStatus: BufferStatus, range: BufferRange, openParen: Rune
): Option[SearchResult] =
  ## If `parenPosition` is an opening paren,
  ## search for the corresponding closing paren and return its position.
  ...
  for i in openParenLine .. range.last.line:
    let
      startColumn = if i == openParenLine: openParenColumn else: 0
      endColumn =
        if i == range.last.line:
          range.last.column
        else:
          buffer[i].high
...
proc searchOpeningParen(
    bufStatus: BufferStatus, range: BufferRange, closeParen: Rune
): Option[SearchResult] =
  ## If `parenPosition` is an closing paren,
  ## Search for the corresponding opening paren and return its position.
    ...
    let
      startColumn =
        if i == closeParenLine:
          ...
          buffer[i].high
      endColumn = if i == range.last.line: range.last.column else: 0
...
proc matchingParenPair*(
    bufStatus: BufferStatus, range: BufferRange, paren: Rune
): Option[SearchResult] =
  ## Return a position of the corresponding pair of paren.

  if bufStatus.buffer.high < range.first.line:
    return
...
proc matchingParenPair*(
  ...
  if isOpenParen(currentRune):
    ...
    let
      ...
      firstPositionColumn =
        if firstPositionLine == parenPosition.line:
          parenPosition.column + 1
        else:
          if bufStatus.buffer[firstPositionLine].high >= 0:
            ...
            0
      firstPosition =
        BufferPosition(line: firstPositionLine, column: firstPositionColumn)

      range = BufferRange(first: firstPosition, last: bufStatus.positionEndOfBuffer)
    ...
    if parenPosition.line == 0 and parenPosition.column == 0:
      return

    let
      # TODO: Add bufStatus.prev or gapbuffer.prev and replace with it.
      firstPositionLine =
        if parenPosition.column > 0:
          parenPosition.line
        else:
          parenPosition.line - 1
      firstPositionColumn =
      ...
      range =
        BufferRange(first: firstPosition, last: BufferPosition(line: 0, column: 0))

    return bufStatus.searchOpeningParen(range, currentRune)

proc currentWord*(
    buffer: seq[Runes], currentPosition: BufferPosition
): tuple[word: Runes, position: int] =
  ## Return the word with position on the current cursor position.
  ## Ignore symbols and spaces.

  template lineBuffer(): Runes =
    buffer[currentPosition.line]

  template currentRune(): Rune =
    lineBuffer[currentPosition.column]

  proc isIgnoreRune(r: Rune): bool {.inline.} =
    (r != '_' and isPunct(r)) or isSpace(r)

  if buffer.len < 1 or lineBuffer.len < 1 or currentPosition.column > lineBuffer.high or
  ...
  return (word: lineBuffer[startCol .. endCol], position: startCol)
...
proc findFirstOfWord*(line: Runes, currentPosition: Natural): Natural =
  ...
  if isDigit(r):
    for i in countdown(currentPosition, 1):
      if not isDigit(line[i - 1]):
        return i
  elif isSpace(r):
    for i in countdown(currentPosition, 1):
      if not isSpace(line[i - 1]):
        return i
  else:
