...
When the editor crashes due to an unhandled exception, or, on Linux, is ended by a deadly signal (`SIGHUP`, `SIGINT`, `SIGQUIT`, `SIGTERM` - closing the terminal window or `kill` sends one), all modified (unsaved) buffers are automatically saved under `~/.cache/moe/crash_recovery/<timestamp>_<pid>/payload/`, one copy per buffer. A copy appears under its final name only once it is complete, as far as the process dying is concerned: nothing is fsynced, so a power loss or kernel panic can still leave a copy that never reached the disk in full. The session's `recovery.json` names the copies, the file each one came from and what ended the session; it is written last, so a session without one is a preserve that did not finish, and its copies are still offered.

On other systems a deadly signal ends moe at once, without preserving, and `:!` runs the command as it always did. On Linux, a signal that arrives during startup ends moe immediately without preserving anything, since there is nothing running yet to save the buffers. While the TUI is suspended for `:!` or `:man`, the command runs as its own job on the terminal: a signal to moe is passed on to it and, when moe holds the terminal, everything it started, and ends the `Press Enter` prompt, so moe gets back to the editor and preserves. A command still running a few seconds later gets `SIGHUP`, as a shell's jobs do when it exits, and then `SIGKILL`. `Ctrl-C` and `Ctrl-\` reach only the command, never moe; Ctrl-Z inside the command is not supported and the command keeps running. After Ctrl-Z on moe itself, `kill %1` preserves and ends it once moe is continued, and `bg` leaves it stopped until `fg`; if the shell exits meanwhile, moe preserves and ends. A stopped moe answers a signal only once it is continued: closing the terminal continues it via the kernel, while `kill %1` or a plain `kill <pid>` stays pending until the next `fg` - the unsaved buffers are kept either way. If moe has to end without getting back to the editor, it still restores the terminal. A signal moe inherited as ignored, as `nohup` leaves `SIGHUP`, stays ignored. moe always ends within 20 seconds of running time after the first signal (time spent stopped does not count): if it does not start saving within 10 seconds, it ends without preserving, and a save still running at 20 seconds is cut short with the copies written so far kept.
...
- **Terminal-Input**: All keystrokes are forwarded to the running shell/command. Press `Ctrl-\ Ctrl-n` to switch to Terminal-Normal sub-mode. A `Ctrl-\` followed by any other key is forwarded to the shell ahead of that key, including the `Ctrl-W` window commands the editor handles itself - the quit character is always spent on the very next keystroke. `Ctrl-\ Ctrl-\` sends one quit character (`SIGQUIT`) to the running program.
